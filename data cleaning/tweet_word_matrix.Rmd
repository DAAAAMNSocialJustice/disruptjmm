...
``` {r}
# Import tweet file
d_tweets.df <- read.csv("~/GitHub/disruptjmm/data cleaning/text of tweets.csv", row.names=1, comment.char="#")
View(id_tweets.df)
```

Now we isolate just the part of the data frame that holds MHALL.

```{r}
# Each page of pdf is one row
# Extracting desired row(s)/page(s)
# Mary Had a Little Lamb is on page 5
mhall <- text_df[5,]
```

**Always a good idea to peek inside 'mhall' and make sure it did what you intended.**  

``` {r}
# Convert desired row(s)/page(s) to character string
mhall <- as.character(mhall)
```
