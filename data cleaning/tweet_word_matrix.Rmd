...
# There was a # nonmen that should be #nonmen
tweets <- str_replace_all(tweets, "\\# ", "\\#")
...
After we look at tweets, we see we need to clean up redundant spaces once again.
...
#tweets_vec<- tweets_ls[1]
```

Now look at `tweets_vec` again. How do you remove the "" entries? 

``` {r}
# Grab just the vector without ""
# First tweets_vec %in% "" finds where they do match
# Then keep just the ones that don't match
#tweets_vec<-tweets_vec[! (tweets_vec %in% "")]
```
...
tweets_el<- cbind(rep(0,length(tweets_ls[[1]])), tweets_ls[[1]])
tweets_el <- as.data.frame(tweets_el)
...
for (i in 2:(length(tweets))) {
  el_row <- cbind(rep(i-1,length(tweets_ls[[i]])), tweets_ls[[i]])
...
tweets.g <- graph_from_edgelist(tweets_el_mat, directed =TRUE)
