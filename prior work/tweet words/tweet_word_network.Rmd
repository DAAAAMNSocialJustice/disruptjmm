...
we clean up redundant spaces & some hanging s
``` {r}
tweets <- str_replace_all(tweets, "s\\s+", " ")
# Replace multiple spaces with one space
# s+ means 1 or more spaces.  \\ are escape characters
tweets <- str_replace_all(tweets, "\\s+", " ")
```
...
# Then keep just the ones that don't match
itweet<-itweet[! (itweet %in% "")] #remove empty cells
...
``` {r}
write.csv(tweets_el, "tweets_el.csv")
