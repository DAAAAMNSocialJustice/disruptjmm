...
For example, 13% of the tweets coded "strategic planning" are also coded "amplification", but only 3% of tweets coded "amplification" are also coded "strategic planning". 

```{r}
tags=c(SOtags,BCtags,BtCtags,CCMtags,SJEDItags)
coincidence <- function(tag1, tag2) {
  t1 <- filter(OCT,grepl(tag1,paste(SO.code,BC.code,BtC.code,CCM.code,SJEDI.code)))
  return(nrow(filter(t1,grepl(tag2,paste(SO.code,BC.code,BtC.code,CCM.code,SJEDI.code))))/nrow(t1))
}
coincidence_df <- outer(tags,tags,Vectorize(coincidence))
colnames(coincidence_df) <- tags
rownames(coincidence_df) <- tags
